...
CREATE TABLE ordenes (
    ...
    CONSTRAINT chk_orden_region_lima CHECK (region_id = 1)
);
...
CREATE TABLE auditoria_sesiones (
    ...
    region_id   INTEGER REFERENCES regiones(id),
    creado_en   TIMESTAMP DEFAULT NOW(),
    CONSTRAINT chk_auditoria_region_lima CHECK (region_id = 1)
);
...
SELECT 'ORD-LIM-0001', u.id, t.id, co.id, cd.id, 1000.00, 3730.00, 3.7300, 'completado', 1
FROM usuarios u
...
WHERE u.email = 'carlos.quispe@gmail.com';

INSERT INTO ordenes (numero_orden, usuario_id, tipo_cambio_id, cuenta_origen_id, cuenta_destino_id, monto_enviado, monto_recibido, tasa_aplicada, estado, region_id)
SELECT 'ORD-LIM-0002', u.id, t.id, co.id, cd.id, 500.00, 1865.00, 3.7300, 'completado', 1
FROM usuarios u
JOIN tipos_cambio t ON t.moneda_origen = 'USD'
JOIN cuentas_bancarias co ON co.usuario_id = u.id AND co.moneda = 'USD'
JOIN cuentas_bancarias cd ON cd.usuario_id = u.id AND cd.moneda = 'PEN'
...
SELECT id, 'COMP-LIM-0001', 'recibo' FROM ordenes WHERE numero_orden = 'ORD-LIM-0001';
